...
test.describe("planning workspace", () => {
  test("checklist, events, timeline, and local persistence", async ({ page }) => {
    ...
    await page.getByLabel("Titre de la tâche").fill("Envoyer le plan d'accès");
    await page.getByLabel("Échéance de la tâche").fill("2026-09-28");
    await page.getByLabel("Personne assignée").selectOption({ label: "Julien" });
    ...
    await page.getByLabel("Titre de l'événement").fill("Essayage final");
    await page.getByLabel("Type d'événement").selectOption("planning");
    await page.getByLabel("Date de l'événement").fill("2027-05-01");
    await page.getByLabel("Lieu de l'événement").fill("Atelier");
    ...
    await page.getByLabel("Heure du moment").fill("14:15");
    await page.getByLabel("Activité").fill("Arrivée des mariés");
    await page.getByLabel("Lieu du moment").fill("Parvis");
    await page.getByLabel("Personne responsable").selectOption({ label: "Camille" });
    await page.getByLabel("Notes du moment").fill("Prévoir dix minutes de battement.");
    await page.getByRole("button", { name: "Ajouter le moment" }).click();
  ...
  test("error state can retry into the checklist", async ({ page }) => {
    ...
    await expect(page.getByRole("alert")).toContainText("Nous n'avons pas pu charger le planning.");
